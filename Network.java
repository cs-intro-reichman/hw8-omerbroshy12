    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String recommendWhoToFollow(String name) 
    {
        ...
        if (currentUser != null) 
        {
            for (int i = 0; i < userCount; i++) 
            {
                User potentialFriend = users[i];
                if (!potentialFriend.getName().equals(name)) 
                {
                    int mutuals = currentUser.countMutual(potentialFriend);
                    if (mutuals > maxMutuals) 
                    {
                        maxMutuals = mutuals;
                        mostRecommended = potentialFriend;
                    }
                }
            }
        }
    ...
    public String mostPopularUser() 
    {
        ...
        for (int i = 0; i < userCount; i++) 
        {
            ...
            int currentFollowers = followeeCount(candidateName);
